...
import cn.embed.media.surfaceEncoder.gles.Drawable2d;
...
public class FilterGroup<T extends IFilter> implements IFilter {
    ...
    public int getTextureTargetGroup() {
    ...
    public void setTextureSize(int width, int height) {
        ...
        for (int i = 0; i < size - 1; i++) {
            ...
            GLES20.glBindTexture(getTextureTargetGroup(), mFrameBufferTextures[i]);
            GlUtil.checkGlError("glBindTexture " + mFrameBufferTextures[i]);

            GLES20.glTexImage2D(GLES20.GL_TEXTURE_2D, 0, GLES20.GL_RGBA, width, height, 0,
                    GLES20.GL_RGBA, GLES20.GL_UNSIGNED_BYTE, null);

            GLES20.glTexParameterf(getTextureTargetGroup(), GLES20.GL_TEXTURE_MAG_FILTER,
                    GLES20.GL_LINEAR);
            GLES20.glTexParameterf(getTextureTargetGroup(), GLES20.GL_TEXTURE_MIN_FILTER,
                    GLES20.GL_LINEAR);
            GLES20.glTexParameterf(getTextureTargetGroup(), GLES20.GL_TEXTURE_WRAP_S,
                    GLES20.GL_CLAMP_TO_EDGE);
            GLES20.glTexParameterf(getTextureTargetGroup(), GLES20.GL_TEXTURE_WRAP_T,
                    GLES20.GL_CLAMP_TO_EDGE);
            ...
            GLES20.glFramebufferTexture2D(GLES20.GL_FRAMEBUFFER, GLES20.GL_COLOR_ATTACHMENT0,
                    GLES20.GL_TEXTURE_2D /*getTextureTargetGroup()*/, mFrameBufferTextures[i], 0);
            ...
            GLES20.glBindTexture(getTextureTargetGroup(), 0);
    ...
    public void onDraw(float[] mvpMatrix, FloatBuffer vertexBuffer, int firstVertex,
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (i == 0) {
                ...
                filter.onDraw(IDENTITY_MATRIX, mDrawable2d.getVertexArray(), firstVertex, vertexCount,
                        coordsPerVertex, vertexStride, IDENTITY_MATRIX,
                        (size % 2 == 0) ? mDrawableFlipVertical2d.getTexCoordArray()
                                : mDrawable2d.getTexCoordArray(), previousTextureId, texStride);
            } else {
                filter.onDraw(IDENTITY_MATRIX, mDrawable2d.getVertexArray(), firstVertex, vertexCount,
                        coordsPerVertex, vertexStride, IDENTITY_MATRIX, mDrawable2d.getTexCoordArray(),
    ...
    public final float[] IDENTITY_MATRIX1 = new float[16];

    public float[] scaleMVPMatrix() {
        Matrix.setIdentityM(IDENTITY_MATRIX1, 0);
        //  Matrix.scaleM(IDENTITY_MATRIX1, 0, 0, 0, 1f);

        Matrix.scaleM(IDENTITY_MATRIX1, 0, 1f, 1f, 1f);
        return IDENTITY_MATRIX1;
    }
