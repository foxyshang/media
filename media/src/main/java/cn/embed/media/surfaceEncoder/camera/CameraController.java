package cn.embed.media.surfaceEncoder.camera;
...
public class CameraController implements Camera.AutoFocusCallback, Camera.ErrorCallback, CommonHandlerListener {
    ...
    private static volatile CameraController sInstance;            //相机的单例模式
    public final static float sCameraRatio = 16f / 9f;            //相机的比例
     ...
     * 打开相机
     *
     * @param surfaceTexture      纹理
     * @param context
     * @param desiredPictureWidth 期望的图片宽度
     */
    public void setupCamera(SurfaceTexture surfaceTexture, Context context, int desiredPictureWidth, boolean mIsCameraBackForward) {
     ...
     * @param param
     */
    public synchronized void setCameraParameters(Camera.Parameters param) {
        if (mCamera != null) {
